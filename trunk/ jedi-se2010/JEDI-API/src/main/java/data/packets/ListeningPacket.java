...
public class ListeningPacket extends Packet {
	
	//type of the id
	private static int TYPE_ID = 30;
	
	public ListeningPacket(int orig, int dest, byte checksum) {
	...
	public static ListeningPacket parseInform(byte[] readBuffer) {
		
		if(readBuffer[2] != TYPE_ID || readBuffer[1] != 0)
			return null;
		
		return new ListeningPacket(readBuffer[0], readBuffer[1], readBuffer[3]);
	}
